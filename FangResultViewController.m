...
- (void)viewDidLoad
{
    ...
    resultBar = [[OANavigationBar alloc]initWithFrame:CGRectMake(0, 0, 320, 44) BackImage:[UIImage imageNamed:@"zx_top.png"]];
    resultBar.title = @"计算结果";
    resultBar.leftButton = [UIButton buttonWithType:UIButtonTypeCustom];
    resultBar.leftButton.frame = CGRectMake(0, 0, 44, 44);
    [resultBar.leftButton setImage:[UIImage imageNamed:@"arrow_02.png"] forState:UIControlStateNormal];
    [resultBar.leftButton addTarget:self action:@selector(backAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:resultBar];
    [resultBar release];
    
    //添加背景滚动视图
    coverScrollview = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 44, 320, self.view.frame.size.height-44)];
...
-(void)addViewTwo
{
    ...
    [expendBtn addTarget:self action:@selector(expendAction:) forControlEvents:UIControlEventTouchUpInside];
